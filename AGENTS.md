...
A personal automation pipeline that turns media into an Obsidian note: it gets a transcript (existing subtitles/captions, or local speech-to-text via Parakeet/MLX when none exist), summarizes it via the Claude CLI, commits the transcript to a GitHub subtitles repo, and commits a markdown note to an Obsidian vault GitHub repo.
...
**Containerized** (optional, README step 9): `pipeline.py`/`fetch_playlist.py`/`server.py` run in Docker (see `Dockerfile`, `docker/docker-compose.yml`) alongside `cloudflared`; only `host_bridge.py` stays native, reached via `host.docker.internal`. Note that `bridge.url` in `config.yaml` must be set appropriately per deployment mode: `http://127.0.0.1:8081` for native, `http://host.docker.internal:8081` for containerized. The example config has it set to the containerized value by default.
