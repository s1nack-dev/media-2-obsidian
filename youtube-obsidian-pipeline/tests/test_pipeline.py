...
@pytest.mark.parametrize("value, expected", [("https://www.youtube.com/watch?v=abc", "youtube"), ("https://example.com/a", "generic_link")])
...
@pytest.mark.parametrize("url, expected", [("https://youtu.be/abc", "abc"), ("https://youtube.com/shorts/xyz", "xyz"), ("https://youtube.com/watch?v=q", "q")])
...
@pytest.mark.parametrize("url, expected", [("https://x.test/a.mp3", True), ("https://x.test/a", False), ("https://x.test/a.MP4", True)])
...
def test_process_local_sidecar_end_to_end(tmp_path, monkeypatch):
    media = tmp_path / "Talk.mp3"; media.write_bytes(b"audio")
    (tmp_path / "Talk.srt").write_text("1\n00:00:00,000 --> 00:00:01,000\nHello world\n")
    subs, vault = tmp_path / "subs", tmp_path / "vault"
    cfg = {"bridge": {"url": "http://bridge", "auth_token_op_ref": "ref"}, "transcription": {},
           "youtube": {"subtitle_languages": ["en"]}, "github": {
               "subtitles_repo_url": "https://github.com/me/subs.git", "subtitles_repo_path": str(subs), "subtitles_branch": "main", "subtitles_dir_in_repo": "transcripts",
               "vault_repo_url": "https://github.com/me/vault.git", "vault_repo_path": str(vault), "vault_branch": "main", "vault_notes_dir": "notes", "commit_author_name": "Test", "commit_author_email": "test@example.com"}}
    monkeypatch.setattr(pipeline, "ensure_repo", lambda *args: Path(args[1]).mkdir(parents=True, exist_ok=True) or Path(args[1]))
    ...
    monkeypatch.setattr(pipeline.bridge_client, "generate_tags", lambda *args: ["testing"])
    result = pipeline.process_input(str(media), cfg, github_token="token", bridge_token="bridge")
...
def test_ytdlp_subtitle_download_prefers_requested_language(tmp_path, monkeypatch):
    ...
    assert pipeline._ytdlp_download_subs("https://example", "vid", ["en", "de"], tmp_path) == (tmp_path / "vid.en.srt", "en")


def test_ytdlp_subtitle_download_falls_back_to_any_language(tmp_path, monkeypatch):
    class R: returncode = 1; stderr = "failed"
    monkeypatch.setattr(pipeline.subprocess, "run", lambda *a, **k: R())
    (tmp_path / "vid.fr.srt").write_text("subtitle")
    assert pipeline._ytdlp_download_subs("https://example", "vid", ["en"], tmp_path)[1] == "fr"


def test_ytdlp_subtitle_timeout_returns_none(tmp_path, monkeypatch):
    monkeypatch.setattr(pipeline.subprocess, "run", lambda *a, **k: (_ for _ in ()).throw(pipeline.subprocess.TimeoutExpired("yt-dlp", 1)))
    assert pipeline._ytdlp_download_subs("https://example", "vid", ["en"], tmp_path) is None
...
def test_audio_download_and_title(monkeypatch, tmp_path):
    ...
    assert pipeline.download_audio_via_ytdlp("https://example", tmp_path).name == "audio.mp3"
...
def test_find_sidecar_supports_common_extensions(tmp_path):
    media = tmp_path / "episode.m4a"; media.touch()
    for suffix in (".vtt", ".txt"):
        sidecar = media.with_suffix(suffix); sidecar.write_text("caption")
        assert pipeline.find_sidecar_subtitle(media) is None
        sidecar.unlink()
...
def test_process_input_raises_when_no_audio_or_subtitles(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(pipeline.bridge_client, "transcribe_audio", lambda *args: (_ for _ in ()).throw(RuntimeError("down")))
